...
function createBaseProviderRegistration(): ProviderRegistration {
  return { owner: "", registeredAt: undefined };
}

export const ProviderRegistration: MessageFns<ProviderRegistration, "akash.provider.v1beta4.ProviderRegistration"> = {
  $type: "akash.provider.v1beta4.ProviderRegistration" as const,

  encode(message: ProviderRegistration, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    if (message.owner !== "") {
      writer.uint32(10).string(message.owner);
    }
    if (message.registeredAt !== undefined) {
      Timestamp.encode(toTimestamp(message.registeredAt), writer.uint32(18).fork()).join();
    }
    return writer;
  },

  decode(input: BinaryReader | Uint8Array, length?: number): ProviderRegistration {
    const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
    const end = length === undefined ? reader.len : reader.pos + length;
    const message = createBaseProviderRegistration();
    while (reader.pos < end) {
      const tag = reader.uint32();
      switch (tag >>> 3) {
        case 1: {
  ...
  fromJSON(object: any): ProviderRegistration {
    return {
      owner: isSet(object.owner) ? globalThis.String(object.owner) : "",
  ...
  toJSON(message: ProviderRegistration): unknown {
    const obj: any = {};
    if (message.owner !== "") {
      obj.owner = message.owner;
    }
  ...
  fromPartial(object: DeepPartial<ProviderRegistration>): ProviderRegistration {
    const message = createBaseProviderRegistration();
    message.owner = object.owner ?? "";
