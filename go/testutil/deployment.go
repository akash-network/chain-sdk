...
func DeploymentGroup(t testing.TB, did dtypes.DeploymentID, gseq uint32) dvbeta.Group {
	t.Helper()
	return dvbeta.Group{
		ID:    dtypes.MakeGroupID(did, gseq),
		State: dvbeta.GroupOpen,
...
// GroupSpec generator
func GroupSpec(t testing.TB) dvbeta.GroupSpec {
	t.Helper()
...
func DeploymentGroups(t testing.TB, did dtypes.DeploymentID, gseq uint32) dvbeta.Groups {
	t.Helper()
	count := rand.Intn(5) + 5 // nolint:gosec
	vals := make(dvbeta.Groups, 0, count)
	for i := 0; i < count; i++ {
