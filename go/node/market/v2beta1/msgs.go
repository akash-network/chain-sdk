...
func NewMsgCreateBid(id BidID, prices []sdk.DecCoin, deposit deposit.Deposit, roffer ResourcesOffer) *MsgCreateBid {
	return &MsgCreateBid{
		ID:             id,
		Prices:         prices,
		Deposit:        deposit,
...
func NewMsgCloseBid(id BidID, reason LeaseClosedReason) *MsgCloseBid {
	return &MsgCloseBid{
		ID:     id,
		Reason: reason,
	}
...
func (msg *MsgCloseBid) ValidateBasic() error {
	...
	if !msg.Reason.IsRange(LeaseClosedReasonRangeProvider) {
		return cerrors.Wrapf(ErrInvalidLeaseClosedReason, "value \"%d\" range 10000..19999", msg.Reason)
	}
