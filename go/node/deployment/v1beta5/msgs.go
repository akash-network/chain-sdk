...
var (
	...
	_ deposit.HasDeposits = &MsgCreateDeployment{}
...
func NewMsgCreateDeployment(
	...
	groups []GroupSpec,
	hash []byte,
	deposit deposit.Deposits) *MsgCreateDeployment {
...
func (msg *MsgCreateDeployment) ValidateBasic() error {
	...
	if err := msg.Deposits.Validate(); err != nil {
		return err
	}
	if len(msg.Groups) == 0 {
	...
	for _, gs := range msg.Groups {
		...
		// one of groupSpec's prices must match the deposit value
		matched := false

	groupSpecs:
		for _, price := range gs.Prices() {
			for _, d := range msg.Deposits {
				if price.Denom == d.Amount.Denom {
					matched = true
					continue groupSpecs
				}
			}
		...
		if !matched {
			return cerrors.Wrapf(v1.ErrInvalidDeposit, "no deposit type matches groupspec price")
		}
	}
