...
func (r *ResourceUnit) FullPrice() sdk.DecCoins {
	res := make(sdk.DecCoins, 0, len(r.Prices))

	for _, price := range r.Prices {
		res = append(res, sdk.NewDecCoinFromDec(price.Denom, price.Amount.MulInt64(int64(r.Count))))
	}
...
func (r *ResourceUnit) validatePricing() error {
	if !r.GetPrices().IsValid() {
		return fmt.Errorf("error: invalid price object")
	}

	for _, price := range r.Prices {
		if price.Amount.GT(sdkmath.LegacyNewDecFromInt(sdkmath.NewIntFromUint64(validationConfig.Unit.Max.Price))) {
			return fmt.Errorf("error: invalid unit price (%v > %v fails)", validationConfig.Unit.Max.Price, price)
		}
	}
