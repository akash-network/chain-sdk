...
var (
	errCertificateNotFoundInPEM = fmt.Errorf("%w: certificate not found in PEM", types.ErrCertificate)
	errPrivateKeyNotFoundInPEM  = fmt.Errorf("%w: private key not found in PEM", types.ErrCertificate)
	errPublicKeyNotFoundInPEM   = fmt.Errorf("%w: public key not found in PEM", types.ErrCertificate)
...
func (kpm *keyPairManager) Read(fin ...io.Reader) ([]byte, []byte, []byte, error) {
	...
	if len(fin) != 0 {
		if len(fin) != 1 {
			return nil, nil, nil, fmt.Errorf("%w: Read() takes exactly 1 or 0 arguments, not %d", types.ErrCertificate, len(fin))
		}
		pemIn = fin[0]
	}
