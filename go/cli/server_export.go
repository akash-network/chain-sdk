...
func ExportCmd(appExporter types.AppExporter, defaultNodeHome string) *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			traceWriterFile, _ := cmd.Flags().GetString(cflags.FlagTraceStore)
	...
	cmd.Flags().Bool(cflags.FlagForZeroHeight, false, "Export state to start at height zero (perform preproccessing)")
