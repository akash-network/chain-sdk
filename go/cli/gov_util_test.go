...
func TestAddGovPropFlagsToCmd(t *testing.T) {
	...
	assert.Regexp(t, `(?m:^\s+--`+cflags.FlagDeposit+` string\s+`+expDepositDesc+`$)`, help, "help output")
	assert.Regexp(t, `(?m:^\s+--`+cflags.FlagMetadata+` string\s+`+expMetadataDesc+`$)`, help, "help output")
	assert.Regexp(t, `(?m:^\s+--`+cflags.FlagTitle+` string\s+`+expTitleDesc+`$)`, help, "help output")
	assert.Regexp(t, `(?m:^\s+--`+cflags.FlagSummary+` string\s+`+expSummaryDesc+`$)`, help, "help output")
}

func TestReadGovPropFlags(t *testing.T) {
	fromAddr := sdk.AccAddress("from_addr___________")
	argDeposit := "--" + cflags.FlagDeposit
	argMetadata := "--" + cflags.FlagMetadata
	argTitle := "--" + cflags.FlagTitle
	argSummary := "--" + cflags.FlagSummary
