...
`,
	...
	cmd.Flags().String(cflags.FlagType, cflags.TypeHash, fmt.Sprintf("The type to be used when querying tx, can be one of \"%s\", \"%s\"", cflags.TypeHeight, cflags.TypeHash))
...
func BootstrapStateCmd(appCreator types.AppCreator) *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if height == 0 {
				home := serverCtx.Viper.GetString(cflags.FlagHome)
