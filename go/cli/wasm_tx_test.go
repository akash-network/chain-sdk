...
import (
	...
	testdata "pkg.akt.dev/go/cli/testdata/wasm"
)
...
func TestParseVerificationFlags(t *testing.T) {
	...
	for name, spec := range specs {
		t.Run(name, func(t *testing.T) {
			flagSet := GetTxGovWasmProposalStoreAndInstantiateContractCmd().Flags()
			require.NoError(t, flagSet.Parse(spec.args))

			gotMsg, err := parseWasmStoreCodeArgs(spec.srcPath, mySender.String(), flagSet)
			require.NoError(t, err)
			require.True(t, ioutils.IsGzip(gotMsg.WASMByteCode))

			gotSource, gotBuilder, gotCodeHash, gotErr := parseWasmVerificationFlags(gotMsg.WASMByteCode, flagSet)
			if spec.expErr {
...
func TestParseAccessConfigFlags(t *testing.T) {
	...
	for name, spec := range specs {
		t.Run(name, func(t *testing.T) {
			flags := GetTxWasmStoreCodeCmd().Flags()
			require.NoError(t, flags.Parse(spec.args))
			gotCfg, gotErr := parseWasmAccessConfigFlags(flags)
			if spec.expErr {
				require.Error(t, gotErr)
				return
			}
