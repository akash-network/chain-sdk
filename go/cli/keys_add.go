...
`,
	...
	f.Bool(cflags.FlagUseLedger, false, "Store a local reference to a private key on a Ledger device")
	...
	f.Bool(cflags.FlagDryRun, false, "Perform action, but don't add key to local keystore")
	...
	f.String(cflags.FlagKeyType, string(hd.Secp256k1Type), "Key signing algorithm to generate keys for")
	...
	f.SetNormalizeFunc(func(f *pflag.FlagSet, name string) pflag.NormalizedName {
		if name == cflags.FlagKeyAlgorithm {
			name = cflags.FlagKeyType
		}
...
func runAddCmd(ctx client.Context, cmd *cobra.Command, args []string, inBuf *bufio.Reader) error {
	...
	algoStr, _ := cmd.Flags().GetString(cflags.FlagKeyType)
	algo, err := keyring.NewSigningAlgoFromString(algoStr, keyringAlgos)
	if err != nil {
		return err
	}

	if dryRun, _ := cmd.Flags().GetBool(cflags.FlagDryRun); dryRun {
	...
	useLedger, _ := cmd.Flags().GetBool(cflags.FlagUseLedger)
